...
#             torch.save(data, saved_data_path)
        ...
        if self.latent_dir is not None:
            ...
            if os.path.exists(latent_path):
                with h5py.File(latent_path, 'r') as f:
                    print('----- new file -----')
                    dac_latents = torch.cat([f[chunk_id]['dac_latents'][()] for chunk_id in f.keys()], dim=1)
    ...
    def __getitem__(self, index):
        if self.preload:
            ...
            result = self.get_segment(
                data, self.hop_length, self.sequence_length)
            result['sr'] = data['sr']
            return result
        else:
    ...
    #     result['audio'] = result['audio'].float().div_(32768.0) # converting to float by dividing it by 2^15
        ...
        result['velocity'] = result['velocity'].float().div_(
            128.0)  # not yet normalized
            ...
            for wavfile, tsvfile in tqdm(self._walker, desc=f'checking downsampled files'):
                try:
                    ...
                    assert os.path.isfile(
                        dsampled_audio), f"{dsampled_audio} is missing"
...
#         self.ext_archive = '.tar'
        ...
        self.groups = self.available_groups(
            groups) if isinstance(groups, str) else list(groups)
        ...
        if self.download:
            ...
            if self._check_all_groups_exist(self.groups):
...
#             with open(a, 'rb') as f:
            test_names = pickle.load(importlib.resources.open_binary(
...
#                     self._walker.extend(wav_paths)
        ...
        for group in groups:
            ...
            if not os.path.isdir(group_path):
                ...
                if group == 'ENSTDkAm':
                    ...
                    check_md5(os.path.join(self.root, self.name_archive,
                              group+'1.zip'), self.hash_dict[group+'1'])
                    check_md5(os.path.join(self.root, self.name_archive,
                              group+'2.zip'), self.hash_dict[group+'2'])
                    extract_archive(os.path.join(
                        self.root, self.name_archive, group+'1.zip'))
                    extract_archive(os.path.join(
                        self.root, self.name_archive, group+'2.zip'))
                else:
                    check_md5(os.path.join(self.root, self.name_archive,
                              group+'.zip'), self.hash_dict[group])
                    extract_archive(os.path.join(
                        self.root, self.name_archive, group+'.zip'))
                print(f' '*50, end='\r')
    ...
    def _check_all_groups_exist(self, groups):
        ...
        for group in groups:
            if os.path.isdir(os.path.join(self.root, self.name_archive, group)):
                ...
                print(
                    f"{group} not found, proceeding to check if the zip file exists", end='\r')
    ...
    def extract_tsv(self):
        ...
        tsvs = glob(os.path.join(self.root, self.name_archive,
                    '*', self.data_type, '*.tsv'))
        num_tsvs = len(tsvs)
        ...
        if decision.lower() == 'yes':
            midis = glob(os.path.join(self.root, self.name_archive,
                         '*', self.data_type, '*.mid'))  # loading lists of midi
            Parallel(n_jobs=multiprocessing.cpu_count())(delayed(process_midi)(
                in_file, out_file) for in_file, out_file in files(midis, output_dir=False))
    ...
    def resample(self, sr, output_format='flac', num_threads=-1):
        ...
        def _resample(wavfile, sr, output_format):
            ...
            sound.export(wavfile.replace(self.original_ext,
                         f'.{output_format}'), format=output_format)

        if num_threads == -1:
            Parallel(n_jobs=multiprocessing.cpu_count())(delayed(_resample)(wavfile, sr, output_format)
...
#         self.url = "https://homes.cs.washington.edu/~thickstn/media/musicnet.tar.gz" # old MusicNet link
        ...
        groups = groups if isinstance(
            groups, list) else self.available_groups(groups)
        self.groups = groups
        ...
        if self.download:
            if os.path.isdir(os.path.join(self.root, self.name_archive)):
                ...
                print(
                    f'{self.name_archive+self.ext_archive} exists, skipping download')
                print(f'Extracting {self.name_archive+self.ext_archive}')
                extract_archive(os.path.join(
                    self.root, self.name_archive+self.ext_archive))
                print(f'Converting csv files into tsv files')
                self.csv2tsv()
            else:
                if not os.path.isdir(self.root):
                ...
                print(f'Downloading from {self.url}')
                download_url(self.url, root,
                             hash_value=self.checksum, hash_type='md5')
...
#               f"of {self.__class__.__name__} at {os.path.join(self.root, self.name_archive)}")
    ...
    def csv2tsv(self):
        ...
        """
        for group in self.groups:
            tsvs = glob(os.path.join(
                self.root, self.name_archive, f"{group}_data", '*.tsv'))
            num_tsvs = len(tsvs)
            if num_tsvs > 0:
                decision = input(f"There are already {num_tsvs} tsv files.\n" +
                                 f"Do you want to overwrite them? [yes/no]")
            ...
            if decision.lower() == 'yes':
                csvs = glob(os.path.join(self.root, self.name_archive,
                            f'{group}_labels', '*.csv'))  # loading lists of csvs
                Parallel(n_jobs=multiprocessing.cpu_count())(delayed(process_csv)(
                    in_file, out_file) for in_file, out_file in files(csvs, output_dir=False))
    ...
    def __init__(self,
        ...
        if self.download:
            ...
            if os.path.isdir(os.path.join(self.root, self.name_archive)):
                ...
                check_md5(os.path.join(self.root, self.name_archive +
                          self.ext_archive), self.hash_dict[self.name_archive])
                tqdm(extract_archive(os.path.join(
                    self.root, self.name_archive+self.ext_archive)))

                check_md5(os.path.join(self.root, self.midi_name_archive +
                          self.ext_archive), self.hash_dict[self.midi_name_archive])
                tqdm(extract_archive(os.path.join(
                ...
                download_url(self.url_dict['maestro-v2.0.0'], self.root,
                             hash_value=self.hash_dict['maestro-v2.0.0'], hash_type='md5')
                download_url(self.url_dict['maestro-v2.0.0-midi'], self.root,
                             hash_value=self.hash_dict['maestro-v2.0.0-midi'], hash_type='md5')
        ...
        elif os.path.isdir(os.path.join(self.root, self.name_archive)):
            pass
        elif os.path.isfile(os.path.join(self.root, 'maestro-v2.0.0.zip')) and os.path.isfile(os.path.join(self.root, 'maestro-v2.0.0-midi.zip')):
...
#                     self._walker.extend(wav_paths)
    ...
    def resample(self, sr, output_format='flac', num_threads=-1):
        ...
        def _resample(wavfile, sr, output_format):  # wavfile is  .flac
            sound = AudioSegment.from_wav(wavfile.replace(
                '.flac', self.original_ext))  # wavfile now is  .wav
            sound = sound.set_frame_rate(sr)  # downsample it to sr
            sound = sound.set_channels(1)  # Convert Stereo to Mono
            sound.export(wavfile.replace(self.original_ext,
                         f'.{output_format}'), format=output_format)
